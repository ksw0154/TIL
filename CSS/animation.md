...
- `keyframe`을 2개만 사용할 때는 `from`과 `to`를 사용할 수 있다.
...
- `keyframe`을 여러개 사용할 경우 `%`를 사용해서 표현할 수 있다.
...
- `@keyframes name`을 작성하면 된다.

#### name 법칙

- 대소문자를 구분한다.
- a to z, 0 to 9, underscores(\_), dashes(-) 사용 가능
- name을 `global values`로 지으면 안된다.
...
- `animation의 한 사이클을 완료하는 데 걸리는 시간을 지정`한다.
...
1. 기존 스타일 (animation이 시작하기 전 스타일)
2. @keyframes 첫 번째 (0%, from)
3. @keyframes 진행 단계 (33%, 25% 등)
4. @keyframes 마지막 (100%, to)
5. 기존 스타일

### `none`

- 기존 스타일로 시작해서 animation 이후 기존 스타일로 돌아온다.
...
- @keyframes의 첫 번째 단계에서 실행되고 나서 기존 스타일로 종료된다.
- 1번에서 시작해서 5번에서 종료된다.

### `both`

- @keyframes의 첫 번째 단계에서 실행되고 @keyframes의 마지막 단계에서 종료된다.
- 기존 스타일은 아예 적용되지 않는다.
...
- `anmation-duration`이 `animation-delay`보다 앞에 작성되어야 한다.

```css
.box {
  /* @keyframes duration | timing-function | delay | iteration-count | direction | fill-mode | play-state | name */
  animation: 3s ease-in-out 1s infinite reverse both running slidein;
}
