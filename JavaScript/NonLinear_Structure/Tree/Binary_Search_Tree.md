...
- 현재 노드를 기준으로 왼쪽에는 작은 값, 오른쪽은 큰 값으로 정렬해 놓은 이진 트리 기반 자료구조

### 구현 메서드

- 노드 추가 : \_insertNode(), insert()
- 노드 탐색 (최대값) : \_maxNode(), max()
- 노드 탐색 (최솟값) : \_minNode(), min()
- 노드 탐색 (특정값) : \_searchNode(), search()
- 노드 삭제 : \_findMinNode(), \_removeNode(), remove()
